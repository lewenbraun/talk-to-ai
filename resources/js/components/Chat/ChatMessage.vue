<template>
  <div :class="messageClass">
    <div v-if="isIncoming" class="w-9 h-9 rounded-full flex items-center justify-center mr-2">
      <img :src="incomingAvatar" alt="User Avatar" class="w-8 h-8 rounded-full">
    </div>
    <div class="flex max-w-96 p-3 rounded-lg" :class="bubbleClass">
      <p>{{ message.content }}</p>
    </div>
    <div v-if="!isIncoming" class="w-9 h-9 rounded-full flex items-center justify-center ml-2">
...
const incomingAvatar = computed(() =>
  "https://placehold.co/200x/ffa8e4/ffffff.svg?text=ʕ•́ᴥ•̀ʔ&font=Lato"
);

const outgoingAvatar = computed(() =>
